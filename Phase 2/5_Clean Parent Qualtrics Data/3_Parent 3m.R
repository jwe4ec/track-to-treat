...
p3m_corrected <- readRDS(file.path(dirs$clean_data_staging_intermediate, "Phase 2 Parent Qualtrics Corrected Data - List by Wave.rds")) %>%
  pluck("p3m")
...
warn_invalid_id_format(p3m_fixed_ids$lsmh_id)


### Remove invalid responses
# Filter to known valid LSMH IDs (marked "keep" in id_lookup) using helper function
p3m_valid_ids <- remove_invalid_p2_qualtrics_responses(p3m_fixed_ids, id_lookup)
...
p3m_valid_ids <- mark_done_in_ax_window(p3m_valid_ids, "3m", ax_windows)
...
saveRDS(p3m_recoded, file.path(dirs$clean_data_staging_intermediate, "Phase 2 Parent Qualtrics Clean Data - 3m.rds"))

# Save log
saveRDS(log, file.path(dirs$clean_data_staging_intermediate, "Phase 2 Parent Qualtrics Clean Data Log - 3m.rds"))
