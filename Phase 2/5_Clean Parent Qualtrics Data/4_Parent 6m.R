...
p6m_corrected <- readRDS(file.path(dirs$clean_data_staging_intermediate, "Phase 2 Parent Qualtrics Corrected Data - List by Wave.rds")) %>%
  pluck("p6m")
...
p6m_valid_ids <- mark_done_in_ax_window(p6m_valid_ids, "6m", ax_windows)
...
saveRDS(p6m_recoded, file.path(dirs$clean_data_staging_intermediate, "Phase 2 Parent Qualtrics Clean Data - 6m.rds"))

# Save log
saveRDS(log, file.path(dirs$clean_data_staging_intermediate, "Phase 2 Parent Qualtrics Clean Data Log - 6m.rds"))
