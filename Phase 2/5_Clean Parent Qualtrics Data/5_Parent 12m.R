...
p12m_corrected <- readRDS(file.path(dirs$clean_data_staging_intermediate, "Phase 2 Parent Qualtrics Corrected Data - List by Wave.rds")) %>%
  pluck("p12m")
...
warn_invalid_id_format(p12m_fixed_ids$lsmh_id)


### Remove invalid responses
# Filter to known valid LSMH IDs (marked "keep" in id_lookup) using helper function
p12m_valid_ids <- remove_invalid_p2_qualtrics_responses(p12m_fixed_ids, id_lookup)
...
p12m_recoded <- p12m_deduplicated %>%
  ...
  mutate(
    ...
    p12m_childtx_lifetime = p12m_childtx_1 == 1 | p12m_childtx_3 == 1,
    p12m_childtx_current = p12m_childtx_3 == 1,
    ...
    p12m_bfamg_mean = mean_across("p12m", "bfamg", name = "p12m_bfamg_mean"),
    
    
    ## BHS-4 (Beck Hopelessness Scale - 4-item) overall mean score
    p12m_bhs_mean = mean_across("p12m", "bhs", name = "p12m_bhs_mean"),
  ...
  select(
    ...
    p12m_in_window_org,
    p12m_in_window_ext,
    p12m_days_before_start_window_12m_org,
    p12m_days_after_end_window_12m_org,
...
walk(items_to_check, check_values, p12m_recoded) # check_values() helper function



####  Save Data  ####
# Save clean Qualtrics data
saveRDS(p12m_recoded, file.path(dirs$clean_data_staging_intermediate, "Phase 2 Parent Qualtrics Clean Data - 12m.rds"))

# Save log
saveRDS(log, file.path(dirs$clean_data_staging_intermediate, "Phase 2 Parent Qualtrics Clean Data Log - 12m.rds"))
